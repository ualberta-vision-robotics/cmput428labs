...
- Our focal length is given in pixels. Convert this to mm using the ratio of our photo's width in pixels and the camera sensor width in pixels.
- Then, convert the focal length in mm to a full-frame equivalent using the diagonal sensor width ratios ([reference your closest sensor dimensions and 35mm film full-frame in this table](https://en.wikipedia.org/wiki/Image_sensor_format))
